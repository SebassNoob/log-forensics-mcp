...
export default function findIocsPrompt({ dirPath, iocs }: InferSchema<typeof schema>) {
	...
	return `Sweep the logs under ${dirPath} for these indicators:
${indicators}
...
3. Call search once per file per indicator. search matches a literal term, so do not pass a regex; for an indicator with several forms (a domain and its IP, a username with and without domain prefix) search each form separately.
...
An indicator is only absent if every file was searched for it. Say so explicitly when coverage is incomplete rather than reporting a clean sweep.`;
}
